...
class login extends CI_Controller {
		...
		{	
			if (get_cookie('remember')) { // check if user activate the "remember me" feature  
				...
				if ( $this->user_model->login($username, $password) )//check username and password correct
				{
					...
					$this->load->view('header_signup'); //if user already logined show main page
				}
			}else{
	...
	public function check_login()
	{
		...
		$hash_password = $this->user_model->decode_password($username,$password);
		if(password_verify($password,$hash_password)){
			if(!$this->session->userdata('logged_in')){	//Check if user already login
				if ( $this->user_model->login($username, $hash_password) )//check username and password
				{
					$user_data = array(
						'username' => $username,
						'logged_in' => true 	//create session variable
